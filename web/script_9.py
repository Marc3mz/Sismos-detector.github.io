...
# # Cargar el modelo
# modelo = joblib.load('modelo_terremotos.joblib')
...
#     #  la función calcular_densidad_sismica esté definida y funcione correctamente
#     # También  agregar el preprocesamiento necesario para el modelo

#     # Reemplaza estas líneas con el preprocesamiento correcto para el modelo
#     df['densidad_sismica'] = 0  # Valor temporal,  calcularlo

#     features = ['latitude', 'longitude', 'magnitude', 'depth', 'densidad_sismica']  # Ajusta según  modelo
#     X = df[features]
...
#         #  datos históricos para entrenar KDE
#         datos_historicos = df.copy()  #  usar datos reales
#         coords_historicos = np.radians(datos_historicos[['latitude', 'longitude']].values)
#         kde.fit(coords_historicos)
        
#         log_densidad = kde.score_samples(coords)
#         df['densidad_sismica'] = np.exp(log_densidad)
#         return df

#     df_pred = calcular_densidad_sismica(df_pred)

#     features = ['latitude', 'longitude', 'magnitude', 'depth', 'densidad_sismica']
#     X = df_pred[features]
